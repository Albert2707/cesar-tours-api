...
export class OrderController {
    static getOrders = async (req: Request, res: Response): Promise<any> => {
        try {
            ...
            let skipValue = (parseInt(skip as string, 10) - 1) * parseInt(limit as string, 10);  // Ajustamos skip
    ...
    static postOrder = async (req: Request, res: Response): Promise<any> => {
        try {
            ...
            // const { name, lastName, email, phone, optionalPhone, countryId } = req.body;
            // const newCustomer = customer.create({ name, lastName, email, phone, optionalPhone, countryId });
            // const customerCreated = await customer.save(newCustomer);
            // console.log(customerCreated);
            const body = req.body as Order;
            const orderToCreate = { ...body, order_num: generateCustomOrderNum() }
            const findOrder = await order.findOne({ where: { order_num: orderToCreate.order_num } })
            if (findOrder) return res.status(400).json({ message: "Order already exists" })
            const newOrder = order.create(orderToCreate);
            const orderCreated = await order.save(newOrder);
            return res.status(201).json({ message: "Order created successfully", orderCreated });
